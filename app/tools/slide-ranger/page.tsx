...
export default function SlideRangerPage() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'assistant',
      content: 'Welcome to Slide Ranger. I can help you create government-style presentations including acquisition briefings, capability reviews, and decision briefings. What presentation do you need?'
    }
  ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/chat`, {
        ...
        body: JSON.stringify({
          message: `[Slide Ranger - Presentation Generation] ${userMessage}`,
          history: messages
        }),
      });
            ...
            <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-lg p-6 sticky top-8">
              ...
              <ul className="space-y-3 text-sm text-slate-400">
                <li className="flex items-start">
                  ...
                  <span>Acquisition strategy briefings</span>
                </li>
                <li className="flex items-start">
                  <span className="text-blue-500 mr-2 mt-0.5">•</span>
                  <span>Program review decks</span>
                </li>
                <li className="flex items-start">
                  <span className="text-blue-500 mr-2 mt-0.5">•</span>
                  <span>Milestone decision briefs</span>
                </li>
                <li className="flex items-start">
                  <span className="text-blue-500 mr-2 mt-0.5">•</span>
                  <span>Market research summaries</span>
                </li>
                <li className="flex items-start">
                  <span className="text-blue-500 mr-2 mt-0.5">•</span>
                  <span>Capability overviews</span>
                </li>
                <li className="flex items-start">
                  <span className="text-blue-500 mr-2 mt-0.5">•</span>
                  <span>Training presentations</span>
                </li>
                <li className="flex items-start">
                  <span className="text-blue-500 mr-2 mt-0.5">•</span>
                  <span>Status update slides</span>
                </li>
              </ul>
            </div>
          </div>
